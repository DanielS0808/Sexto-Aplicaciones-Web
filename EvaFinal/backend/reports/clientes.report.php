...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 15, 'Reporte de Clientes', 0, 1, 'C', true);
    ...
    function TablaClientes($header, $data) {        
        ...
        $w = array(10, 30, 30, 30, 30, 25, 35); #Ancho de las columnas        
        #Cabeceras
        ...
        $fill = false;        
        foreach ($data as $row) {
            $this->Cell($w[0], 6, $row['cli_id'], 'LR', 0, 'L', $fill);
            $this->Cell($w[1], 6, $row['cli_cod'], 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $row['cli_nom'], 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, $row['cli_ape'], 'LR', 0, 'L', $fill);
            $this->Cell($w[4], 6, $row['cli_dir'], 'LR', 0, 'L', $fill);
            $this->Cell($w[5], 6, $row['cli_cel'], 'LR', 0, 'L', $fill);
            $this->Cell($w[6], 6, $row['cli_ema'], 'LR', 0, 'L', $fill);
            $this->Ln();
            $fill = !$fill;
        }        
...
$pdf->AliasNbPages();
$pdf->AddPage();
$header = array('ID', 'Codigo', 'Nombre', 'Apellido', 'Direccion', 'Celular', 'Email');
